...
static int psci_to_dev_err(int err)
{
	switch (errno) {
...
static int psci_api_cpu_off(const struct device *dev, uint32_t state)
{
	...
	err = data->invoke_psci_fn(PSCI_0_2_FN_CPU_OFF, state, 0, 0);

	return psci_to_dev_err(err);
}
			   ...
			   unsigned long entry_point)
{
	...
	err = data->invoke_psci_fn(PSCI_FN_NATIVE(0_2, CPU_ON), cpuid,
				   entry_point, 0);
